    ...
    def _dp_scan(self, log_potentials, lengths=None, force_grad=False):
        ...
        for b in range(lengths.shape[0]):
            ...
            chart[1][:, b, point:bin_MN // 2, ind, ind, Mid] = \
                    semiring.one_(chart[1][:, b, point:bin_MN // 2, ind, ind, Mid])
            point = (end + M) // 2
            lim = point *2
            chart[0][:, b, rot_x[:end + M], rot_y[:lim], rot_y[:lim], :] = \
        ...
        for b in range(lengths.shape[0]):
            ...
            point = (end + M) // 2
            lim = point *2
            chart[1][:, b, :point, ind_M, ind_M, :] =\
                torch.stack(
                    [chart[0][:, b, :lim:2,  ind_M, ind_M, Down],
                     semiring.sum(
                         torch.stack([chart[0][:, b, :lim:2,  ind_M, ind_M, Mid],
                                      chart[0][:, b, 1:lim:2, ind_M, ind_M, Mid]], dim=-1)),
                     chart[0][:, b, :lim:2,  ind_M, ind_M, Up]], dim =-1)

            x = torch.stack([ind_U, ind_D], dim=0)
            y = torch.stack([ind_D, ind_U], dim=0)
            q = torch.stack([semiring.times(chart[0][:, b, :lim:2,  ind_D, ind_D, :],
                                            chart[0][:, b, 1:lim:2, ind_U, ind_U, Down:Down+1]),
                             semiring.times(chart[0][:, b, :lim:2,   ind_U, ind_U, :],
                                            chart[0][:, b, 1:lim:2,  ind_D, ind_D, Up:Up+1])], dim = 2)
