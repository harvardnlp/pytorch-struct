...
def params_l3():
    ...
    terms = torch.tensor(terms, dtype=torch.float64, 
        requires_grad=True).unsqueeze(0).float()
    roots = torch.tensor(roots, dtype=torch.float64, 
        requires_grad=True).unsqueeze(0).float()
    rules = torch.tensor(rules, dtype=torch.float64, 
        requires_grad=True).unsqueeze(0).float()
