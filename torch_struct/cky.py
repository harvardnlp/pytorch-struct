...
def cky_inside(terms, rules, roots, semiring=LogSemiring):
    ...
    beta = [_make_chart((batch_size, N, N, NT + T), rules, semiring)
            for _ in range(2)]

    span = [_make_chart((batch_size, N, NT + T), rules, semiring)
            for _ in range(N)]
...
def cky_check(terms, rules, roots, semiring=LogSemiring):
                    ...
                    for z in range(NT) if w != end-1 else range(NT, NT+T):
                        ...
                        for m1, y1 in enumerate(y, start, w):
                            for m2, z1 in enumerate(z, w, end):
                                yield (semiring.times(semiring.times(m1, m2),
                                                      rules[0, x, y, z]),
                                       [(x, start, w, end)] + y1 + z1)
    ...
    ls = []
    for nt in range(NT):
        ls += [semiring.times(s, roots[0, nt])
               for s, _ in enumerate(nt, 0, N)]
